...
public sealed class SqlServerArchiveCutoffAdvancer<TStore> : BaseArchiveCutoffAdvancer<TStore> where TStore : SqlServerEventStoreOptions
{
    private readonly TStore _options;

    public SqlServerArchiveCutoffAdvancer(SqlServerConnectionFactory<TStore> factory, TStore options) : base(factory, options)
    {
        _options = options;
    }
...
WHERE domain = @Domain AND stream_id = @StreamId AND (ArchiveCutoffVersion IS NULL OR ArchiveCutoffVersion < @NewCutoff);",
            new { NewCutoff = newCutoffVersion, Domain = domain, StreamId = Guid.Parse(streamId) }, cancellationToken: ct);
