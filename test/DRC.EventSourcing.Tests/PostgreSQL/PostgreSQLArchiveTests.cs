...
/// <summary>
/// Tests for PostgreSQL archive coordinator functionality including cold/hot storage archival,
/// retention policies, and the combined event feed.
/// NOTE: These tests require a running PostgreSQL instance.
/// </summary>
public class PostgreSQLArchiveTests : EventStoreTestBase
{
    ...
    private readonly PostgreSQLArchiveCoordinator<TestPostgreSQLOptions> _archiveCoordinator;
    private readonly PostgreSQLSnapshotStore<TestPostgreSQLOptions> _snapshotStore;
    private readonly TestSnapshotCoordinator _snapshotCoordinator;
    private readonly PostgreSQLArchiveCutoffAdvancer<TestPostgreSQLOptions> _cutoffAdvancer;
    private readonly FileColdEventArchive _coldArchive;
    private readonly PostgreSQLArchiveSegmentStore<TestPostgreSQLOptions> _segmentStore;
    private readonly PostgreSQLCombinedEventFeed<TestPostgreSQLOptions> _combinedFeed;
    private readonly DefaultDomainRetentionPolicyProvider _policyProvider;
    private readonly string _testStoreName;

    public PostgreSQLArchiveTests()
    {
        ...
        var schemaInitializer = new PostgreSQLSchemaInitializer<TestPostgreSQLOptions>(
            _connectionFactory, 
            _options);

        _archiveCoordinator = new PostgreSQLArchiveCoordinator<TestPostgreSQLOptions>(
            _connectionFactory,
            _options);

        _snapshotStore = new PostgreSQLSnapshotStore<TestPostgreSQLOptions>(
            _connectionFactory,
            _options);

        _cutoffAdvancer = new PostgreSQLArchiveCutoffAdvancer<TestPostgreSQLOptions>(
            _connectionFactory,
            _options);
        ...
        _segmentStore = new PostgreSQLArchiveSegmentStore<TestPostgreSQLOptions>(
            _connectionFactory,
            _options);

        _combinedFeed = new PostgreSQLCombinedEventFeed<TestPostgreSQLOptions>(
            _eventStore,
            _coldArchive,
            _segmentStore);
    ...
    public async Task Archive_HardDeletable_ShouldDeleteEventsAndStream()
    {
        ...
        using var conn = (NpgsqlConnection)_connectionFactory.CreateConnection();
        await conn.OpenAsync();
        await conn.ExecuteAsync(
            $"UPDATE {_options.StreamsTableName} SET IsDeleted = true WHERE domain = @Domain AND stream_id = @StreamId",
